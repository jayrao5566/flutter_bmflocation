...
class _MyAppState extends State<PolygonGeofencePage> {
  ...
  void _addGeofence() {
    if (_coordinateList.length < 3) {
      ...
      List<BMFLocCoordinate> _locCoordinateList = [];

      for (var element in _coordinateList) {
        _locCoordinateList.add(BMFLocCoordinate(element.latitude, element.longitude));
      }
      LocationPolygonGeofenceOption op = LocationPolygonGeofenceOption(
          ...
          coordinateList: _locCoordinateList);
