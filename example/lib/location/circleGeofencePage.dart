  ...
  void _addGeofence() {
    if (_coordinate.longitude > 0 || _coordinate.latitude > 0) {
      ...
      LocationCircleGeofenceOption op = LocationCircleGeofenceOption(
          ...
          centerCoordinate: BMFLocCoordinate(_coordinate.latitude, _coordinate.longitude),
      ...
      _myGeofencePlugin.didGeoFencesStatusChangedCallback(
        ...
        if (geofenceResult.isNotEmpty) {
          setState(() {
            ...
            // warning-自定位id为空的情况
            String fenceId = map['fenceId'];
            String customId = map['customId'];
            print('地理围栏的fenceId:$fenceId\n地理围栏的customId:$customId');
            switch (status) {
              case 1:
                resultText = '进入地理围栏';
                break;
